...
def get_token_from_input() -> Optional[str]:
    ...
    try:
        token = getpass.getpass("Enter your HypeRate API token (input will be hidden): ").strip()
        if not token:
...
Examples:
  ...
  python Tests/run_real_integration.py --token=your_token -k test_connection
    ...
    parser.add_argument(
        '--token', 
        type=str, 
        help='HypeRate API token'
    )
    
    parser.add_argument(
        '--interactive', 
        action='store_true',
        help='Prompt for API token interactively'
    )
    ...
    if not token:
        ...
        print("2. Use command line: python Tests/run_real_integration.py --token=your_token")
